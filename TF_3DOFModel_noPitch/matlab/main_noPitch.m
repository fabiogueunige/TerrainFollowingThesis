...
for k = 2:N
    ...
    [z_meas(:,k), hmes, prob(:,k)] = measurament(beta, qt, Gamma, Lambda, u(:,k-1), Ts, prob(:,k-1), theta);
    z_meas(:,k) = z_meas(:,k) + v;
    ...
    %% --- EKF: Prediction ---
    x_pred = f(x_est(:,k-1), u(:,k-1), Ts, theta) + w; % State prediction
    F = jacobian_f(x_est(:,k-1), u(:,k-1), Ts, theta); % Dynamics Jacobian
    P_pred = F * P * F' + Q;
    % fprintf('P_pre = [%.2f , %.2f ;\n        %.2f , %.2f]\n', P_pred(1,1), P_pred(1,2), P_pred(2,1),P_pred(2,2)')

    %% --- EKF: Update ---
    H = jacobian_h(x_pred, Gamma, Lambda, theta);   % Observation Jacobian
    if any(isnan(H(:)))
        H
    ...
    z_pred = h(x_pred, Gamma, Lambda, theta);        % Predicted measurement
