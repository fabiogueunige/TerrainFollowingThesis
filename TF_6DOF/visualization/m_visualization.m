function m_visualization(pr, pplane, n, num_s, p_int, wRr, j, planes, plane_contact_idx, sensor_valid)
    ...
    figure('Name', sprintf('AUV navigation at %d ms', j), 'Tag', sprintf('auv_situation_%d', j), 'NumberTitle', 'off'); 
    ...
    Lx = 2.5;   % length along robot x (era 0.8)
    Wy = 1.2;   % width along robot y (era 0.4)
    Hz = 0.7;   % height along robot z (era 0.2)
    ...
    plot3([pr(1), p_x_dir(1)], [pr(2), p_x_dir(2)], [pr(3), p_x_dir(3)], 'r-', 'LineWidth', 2.5, 'DisplayName', 'X_r');
    plot3([pr(1), p_y_dir(1)], [pr(2), p_y_dir(2)], [pr(3), p_y_dir(3)], 'g-', 'LineWidth', 2.5, 'DisplayName', 'Y_r');
    plot3([pr(1), p_z_dir(1)], [pr(2), p_z_dir(2)], [pr(3), p_z_dir(3)], 'b-', 'LineWidth', 2.5, 'DisplayName', 'Z_r');
    ...
    for k = 1:num_s
        ...
        if sensor_valid(k)
            plot3([pr(1), p_int(1, k)], [pr(2), p_int(2, k)], [pr(3), p_int(3, k)], ...
                  'LineWidth', 3, 'Color', colors(k, :), 'DisplayName', sprintf('Ray y%d', k));
            plot3(p_int(1, k), p_int(2, k), p_int(3, k), 'x', 'Color', colors(k, :), ...
                    'MarkerSize', 10, 'LineWidth', 2, 'DisplayName', sprintf('Int y%d', k));
        end
    end
    ...
    if numel(n) == 3 && abs(norm(n) - 1) < 1e-3
        ...
        plot3(p_proj(1), p_proj(2), p_proj(3), 'kd', 'MarkerSize', 6, 'LineWidth', 2.5, 'DisplayName', 'Proj(h)');
        % Normal arrow
        quiver3(p_proj(1), p_proj(2), p_proj(3), n(1), n(2), n(3), 'r', 'LineWidth', 3, 'MaxHeadSize', 2);
    end
    ...
    xlabel('Axis X (m)'); ylabel('Axis Y (m)'); zlabel('Axis Z (m)');
    set(gca, 'YDir', 'reverse', 'ZDir', 'reverse');
    % Limiti fissi per la visualizzazione
    % xlim([-5 30]);
    % ylim([-12 12]);
    % zlim([-3 15]);
    title(sprintf('AUV navigation at %d ms', j));
    axis equal;
    view(3); % x verso destra
    hold off;
end
