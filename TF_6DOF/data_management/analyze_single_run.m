...
% Plots are saved automatically as PNG (300 DPI) and PDF (vector) for thesis.
%
% SYNTAX:
...
%     'save_plots' - true/false to save plots as PNG+PDF (default: false)
%                    Plots saved to: results/<run_name>/plots/
%
% WHAT IT DOES:
...
function analyze_single_run(varargin)
...
function generate_all_plots(sim_data, run_name, save_plots)
    ...
    function save_figure(fig, filename)
        if save_plots
            ...
            exportgraphics(fig, fullfile(plots_dir, [filename '.png']), 'Resolution', 300);
        end
    end
    ...
    save_figure(fig1, 'state_machine');
    ...
    save_figure(fig2, 'trajectory_3d');
    ...
    save_figure(fig3, 'altitude_tracking');
    ...
    save_figure(fig4, 'terrain_alpha');
    ...
    for i = 1:6
        ...
        save_figure(fig, sprintf('control_%d_%s', i, control_tags{i}));
    end
    ...
    save_figure(fig15, 'normal_parallelism');
    ...
    save_figure(fig17, 'sbes_measurements');
    ...
    if isfield(sim_data, 'x_loc') && isfield(sim_data, 'eta_gt') && ~all(sim_data.eta_gt(:) == 0)
        ...
        save_figure(fig20, 'ekf_pos_velocities');
        ...
        save_figure(fig21, 'ekf_pos_errors');
    end
    ...
    save_figure(fig22, 'sensor_failures');
    
    % Final message
    if save_plots
        fprintf('\n  All figures saved (PNG + PDF) to: %s\n', plots_dir);
    end
