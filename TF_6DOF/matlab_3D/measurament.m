function [ymes, h_real, pr] = measurament(alpha, beta, pplane, n0, Gamma, Lambda, Eta, Zeta, s, num_s, ...
    ...
    for j = 1:num_s
        if (norm(s(:,j)) ~= 1)
            fprintf('ERROR: norm sensor k = %.0f is not 1', j);
        end
    end
    ...
    fprintf('phi new: %.2f | p_gyr: %.2f \n', rad2deg(phm), p_gyr);
    fprintf('theta new: %.2f | q_gyr: %.2f \n', rad2deg(thm), q_gyr);
    fprintf('Punto x: %.2f | Punto y: %.2f | Punto z: %.2f\n', pr(1), pr(2), pr(3));
end