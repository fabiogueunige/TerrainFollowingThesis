...
w_dim = 6;          % number of world dimensions
...
prob_mes = zeros(d_dim,N);          % AUV initial position for measuraments
...
for k = 2:N
    ...
    [clean_rot(:,k), rob_rot(:,k), wRr_real] = AHRS_measurement(cart_pos(:,k), Ts, v_a, clean_rot(:,k-1), u(:,k-1));
    [z_meas(:,k), hmes, prob_mes(:,k), R(:,:,k)] = measurament(alpha, beta, pplane, n0, r_s, s_dim, u(:,k), Ts,  ...
                                                   prob_mes(:,k-1), wRr_real, k, R(:,:,k));
...
% Points
figure;
scatter3(prob_mes(1,:), prob_mes(2,:), prob_mes(3,:), [], time);
colorbar; 
colormap(jet);
xlabel('On X');
ylabel('On Z');
